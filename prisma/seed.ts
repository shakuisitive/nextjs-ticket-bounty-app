...
const users = [
  ...
  { username: "user", email: "shakirkhan72727@gmail.com" },
];
...
const seed = async () => {
  ...
  const [createdTicket] = await prisma.ticket.createManyAndReturn({
    data: tickets.map((ticket) => ({ ...ticket, userId: createdUser.id })),
  });
